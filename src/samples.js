...
export const SAMPLE_CIRCUITS = {
  ...
  // ---- 幅 (バス) を使う例 ----
  // 入力・定数・メモリは幅を持てる (行の 8 番目が幅)。ゲートと出力は繋いだ先から
  // 幅が伝わるので、置き方は 1 ビットのときと変わらない。
  '4 ビットのビット演算 (バス)': {
    nodes: [
      [1, 'in', 40, 90, 10, 'a', null, 4],
      [2, 'in', 40, 250, 12, 'b', null, 4],
      [3, 'and', 300, 100],
      [4, 'xor', 300, 240],
      [5, 'out', 560, 100, 0, 'm'],
      [6, 'out', 560, 240, 0, 'x'],
    ...
    wires: [[1, 0, 3, 0], [2, 0, 3, 1], [1, 0, 4, 0], [2, 0, 4, 1], [3, 0, 5, 0], [4, 0, 6, 0]],
    seq: [
      { set: { a: 10, b: 12 }, expect: { m: 8, x: 6 } },     // A & C = 8 / A ^ C = 6
      { set: { a: 15, b: 0 }, expect: { m: 0, x: 15 } },
    ],
  },
  '4 ビットメモリのトグル (バス)': {
    nodes: [
      [1, 'dff', 330, 180, 0, 'q', null, 4],
      [2, 'not', 540, 180],
      [3, 'out', 710, 180, 0, 'out'],
    ],
    // 4 ビットまとめて反転して自分に戻す。0 → F → 0 …
    wires: [[1, 0, 2, 0], [2, 0, 1, 0], [1, 0, 3, 0]],
    seq: [
      { expect: { out: 0 } },
      { clock: 1, expect: { out: 15 } },
      { clock: 1, expect: { out: 0 } },
      { clock: 3, expect: { out: 15 } },
    ],
  },
  // ---- 幅を混ぜる / 算術の部品 ----
  '4 ビット加算器と比較 (バス)': {
    nodes: [
      [1, 'in', 40, 60, 9, 'a', null, 4],
      [2, 'in', 40, 210, 5, 'b', null, 4],
      [3, 'add', 280, 60],
      [4, 'lt', 280, 210],
      [5, 'out', 540, 60, 0, 'sum'],
      [6, 'out', 540, 210, 0, 'less'],
    ],
    wires: [[1, 0, 3, 0], [2, 0, 3, 1], [1, 0, 4, 0], [2, 0, 4, 1], [3, 0, 5, 0], [4, 0, 6, 0]],
    seq: [
      { set: { a: 9, b: 5 }, expect: { sum: 14, less: 0 } },
      { set: { a: 3, b: 5 }, expect: { sum: 8, less: 1 } },
      { set: { a: 15, b: 1 }, expect: { sum: 0, less: 0 } },   // 4 ビットなので桁上げは捨てる
    ],
  },
  'バスから 1 ビット取り出して束ね直す': {
    nodes: [
      [1, 'in', 40, 130, 6, 'a', null, 4],
      [2, 'bit', 260, 40, 0],        // a[0]
      [3, 'bit', 260, 220, 3],       // a[3]
      [4, 'cat', 470, 130],          // {a[0], a[3]} → 2 ビット
      [5, 'out', 680, 130, 0, 'sw'],
    ],
    // 最下位と最上位を入れ替えて 2 ビットにする
    wires: [[1, 0, 2, 0], [1, 0, 3, 0], [2, 0, 4, 0], [3, 0, 4, 1], [4, 0, 5, 0]],
    seq: [
      { set: { a: 6 }, expect: { sw: 0 } },     // 0110 → {0, 0} = 00
      { set: { a: 9 }, expect: { sw: 3 } },     // 1001 → {1, 1} = 11
      { set: { a: 1 }, expect: { sw: 2 } },     // 0001 → {1, 0} = 10
      { set: { a: 8 }, expect: { sw: 1 } },     // 1000 → {0, 1} = 01
    ],
  },
  'バスを上下に割って入れ替える (部分選択)': {
    nodes: [
      [1, 'in', 40, 130, 0xa5, 'a', null, 8],
      // ビット取り出しは「幅」を付けると範囲になる: 下は a[3:0]、上は a[7:4]
      [2, 'bit', 280, 40, 0, null, null, 4],
      [3, 'bit', 280, 220, 4, null, null, 4],
      [4, 'cat', 500, 130],          // {a[3:0], a[7:4]} → 8 ビット
      [5, 'out', 710, 130, 0, 'sw'],
    ],
    wires: [[1, 0, 2, 0], [1, 0, 3, 0], [2, 0, 4, 0], [3, 0, 4, 1], [4, 0, 5, 0]],
    seq: [
      { set: { a: 0xa5 }, expect: { sw: 0x5a } },   // A5 の上下を入れ替えて 5A
      { set: { a: 0x0f }, expect: { sw: 0xf0 } },
      { set: { a: 0x12 }, expect: { sw: 0x21 } },
    ],
  },
  'スイッチ 4 つを 1 本のバスにする (多入力の連接)': {
    nodes: [
      [1, 'in', 40, 30, 1, 'b3'],
      [2, 'in', 40, 110, 0, 'b2'],
      [3, 'in', 40, 190, 1, 'b1'],
      [4, 'in', 40, 270, 0, 'b0'],
      // 連接の「幅」の欄は入力の本数。4 本にすると {b3, b2, b1, b0} になる
      [5, 'cat', 300, 110, 0, null, null, 4],
      [6, 'out', 520, 150, 0, 'bus'],
      // 束ねた 4 ビットをそのまま数として使える。8 以上か = 最上位が立っているか
      [7, 'const', 300, 320, 8, null, null, 4],
      [8, 'lt', 520, 260],
      [9, 'out', 730, 260, 0, 'small'],
    ],
    wires: [
      [1, 0, 5, 0], [2, 0, 5, 1], [3, 0, 5, 2], [4, 0, 5, 3],
      [5, 0, 6, 0], [5, 0, 8, 0], [7, 0, 8, 1], [8, 0, 9, 0],
    ],
    seq: [
      { set: { b3: 1, b2: 0, b1: 1, b0: 0 }, expect: { bus: 0xa, small: 0 } },
      { set: { b3: 0, b2: 1, b1: 1, b0: 1 }, expect: { bus: 0x7, small: 1 } },
      { set: { b3: 1, b2: 1, b1: 1, b0: 1 }, expect: { bus: 0xf, small: 0 } },
      { set: { b3: 0, b2: 0, b1: 0, b0: 1 }, expect: { bus: 0x1, small: 1 } },
    ],
  },
  '絶対差 |a-b| (減算・小なり・選択)': {
    nodes: [
      [1, 'in', 40, 60, 9, 'a', null, 4],
      [2, 'in', 40, 280, 5, 'b', null, 4],
      [3, 'lt', 270, 40],           // a < b ?
      [4, 'sub', 270, 150],         // b - a
      [5, 'sub', 270, 290],         // a - b
      [6, 'mux', 500, 130],         // 上から 選択信号 / 1 のとき / 0 のとき
      [7, 'out', 730, 150, 0, 'diff'],
    ],
    wires: [
      [1, 0, 3, 0], [2, 0, 3, 1],
      [2, 0, 4, 0], [1, 0, 4, 1],
      [1, 0, 5, 0], [2, 0, 5, 1],
      [3, 0, 6, 0], [4, 0, 6, 1], [5, 0, 6, 2],
      [6, 0, 7, 0],
    ],
    seq: [
      { set: { a: 9, b: 5 }, expect: { diff: 4 } },
      { set: { a: 3, b: 5 }, expect: { diff: 2 } },
      { set: { a: 7, b: 7 }, expect: { diff: 0 } },
      { set: { a: 0, b: 15 }, expect: { diff: 15 } },
    ],
  },
  'NOR / XNOR と一致 (1 ビットでは XNOR = 一致)': {
    nodes: [
      [1, 'in', 40, 80, 0, 'a'],
      [2, 'in', 40, 240, 0, 'b'],
      [3, 'nor', 280, 50],
      [4, 'xnor', 280, 160],
      [5, 'eq', 280, 270],
      [6, 'out', 520, 50, 0, 'nor_ab'],
      [7, 'out', 520, 160, 0, 'xnor_ab'],
      [8, 'out', 520, 270, 0, 'eq_ab'],
    ],
    wires: [
      [1, 0, 3, 0], [2, 0, 3, 1], [3, 0, 6, 0],
      [1, 0, 4, 0], [2, 0, 4, 1], [4, 0, 7, 0],
      [1, 0, 5, 0], [2, 0, 5, 1], [5, 0, 8, 0],
    ],
    // 幅 1 なら「両方 0 でない」= 一致 なので、xnor_ab と eq_ab は必ず同じになる
    seq: [
      { set: { a: 0, b: 0 }, expect: { nor_ab: 1, xnor_ab: 1, eq_ab: 1 } },
      { set: { a: 1, b: 0 }, expect: { nor_ab: 0, xnor_ab: 0, eq_ab: 0 } },
      { set: { a: 0, b: 1 }, expect: { nor_ab: 0, xnor_ab: 0, eq_ab: 0 } },
      { set: { a: 1, b: 1 }, expect: { nor_ab: 0, xnor_ab: 1, eq_ab: 1 } },
    ],
  },
