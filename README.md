...
| [src/schematic.js](src/schematic.js) | 回路グラフ → Verilog・保存形式（GUI エディタ用フロントエンド） | 419 |
...
`GUI 回路グラフ` はエディタのサンプル回路（[src/schematic.js](src/schematic.js) の `SAMPLE_CIRCUITS`）を Verilog に変換して突き合わせる。組合せ回路は真理値表、メモリ入りの回路は入力を変えながらクロックを打った値の列で確認する。未配線の部品が下流ごと除外されること、フィードバック配線が組合せループとして弾かれる一方で**メモリを挟んだ帰還は通る**ことも見ている。

### 回路エディタの操作テスト

[test/ui.js](test/ui.js) は headless Chrome を立ち上げて、**本物のマウス・キーボード入力**を送り込む。207 件。
...
**[web/editor.html](web/editor.html) — GUI 回路エディタ。** 1 ビットの入出力・論理ゲート・定数（0 / 1）・1 ビットのメモリ（DFF）を置いて配線する。操作は 4 つだけ。
...
サンプルは AND / 半加算器 / NAND 4 個で XOR / 多数決 / 組合せループ（エラーになる例）/ 1 ビットメモリ 2 種 / **4 ビットバレルシフタ**。バレルシフタは 2:1 マルチプレクサの 2 段で任意のシフト量を 1 パスで作る回路で、27 ゲート・6 入力・4 出力になる。手で置くには大きいので [src/schematic.js](src/schematic.js) の中で組み立てている。押し出された桁は捨て、空いた桁には定数 0 を入れる（論理左シフト）。
...
- **回路の部品化**：保存した回路を 1 個の部品として置けるようにする。コンパイラ側の「モジュール階層」と同じ仕事なので、GUI とコンパイラの未対応項目を 1 つの実装で埋められる
- 波形の値をクリックした所で読むカーソル
